...
if (isset($_POST['saveData'])) {
    ...
    if (empty($_POST['finesDesc']) OR empty($debet)) {
        ...
        $data['debet'] = (string)str_replace(',', '.', $debet);
        $data['credit'] = (string)str_replace(',', '.', $credit);
...
if ((isset($_GET['detail']) && isset($_GET['itemID'])) || (isset($_GET['action']) && $_GET['action'] == 'detail')) {
    ...
    function debetCurrency($db, $data)
    {
        global $sysconf;
        $currencySettings = $sysconf['currencySetting']??[];
        return ($currencySettings['code']??'') . ' ' . number_format($data[3], $currencySettings['decimal']??0, ',', '.');
    }

    function creditCurrency($db, $data)
    {
        global $sysconf;
        $currencySettings = $sysconf['currencySetting']??[];
        return ($currencySettings['code']??'') . ' ' . number_format($data[4], $currencySettings['decimal']??0, ',', '.');
    }

    $datagrid->modifyColumnContent(3, 'callback{debetCurrency}');
    $datagrid->modifyColumnContent(4, 'callback{creditCurrency}');
