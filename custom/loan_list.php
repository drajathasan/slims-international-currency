...
if (isset($_SESSION['memberID'])) {
    ...
    while ($loan_list_data = $loan_list_query->fetch_assoc()) {
        ...
        if ($overdue) {
            ...
            $loan_list_data['title'] .= ' - <strong class="text-danger">'.__('OVERDUED for').' '.$overdue['days'].' '.__('days(s) with fines value').' '. ($currencySettings['code']??'') . ' ' .number_format($overdue['value'], $currencySettings['decimal']??0, ',', '.').'</strong>'; //mfc
    ...
    if ($is_overdue) {
        echo '  <div class="alert alert-danger p-2 rounded-0">
                    <div class="row">
                        <div class="pt-2 col"><strong>'.__('Total of temporary fines').' is '.($currencySettings['code']??'') . ' ' .number_format($_total_temp_fines, $currencySettings['decimal']??0, ',', '.').'</strong></div>
